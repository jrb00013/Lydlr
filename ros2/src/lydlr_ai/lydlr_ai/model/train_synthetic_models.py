...
class AdvancedCompressionTrainer:
    ...
    def __init__(self, model_dir="models", version=None, model_name_prefix=None):
        ...
        # Prompt for model name prefix if not provided
        if model_name_prefix is None:
            print("\n" + "="*60)
            print("📝 Model Naming")
            print("="*60)
            default_prefix = "lydlr_compressor"
            user_input = input(f"Enter model name prefix (default: '{default_prefix}'): ").strip()
            self.model_name_prefix = user_input if user_input else default_prefix
            print(f"✅ Using model prefix: {self.model_name_prefix}")
            print("="*60 + "\n")
        else:
    ...
    def save_multimodal_model(self):
        ...
        model_path = self.model_dir / f"{self.model_name_prefix}_v{self.version}.pth"
        metadata_path = self.model_dir / f"metadata_{self.model_name_prefix}_v{self.version}.json"
    ...
    def save_sensor_motor_model(self):
        ...
        # Use the same prefix but with sensor_motor suffix
        sensor_motor_prefix = self.model_name_prefix.replace("compressor", "sensor_motor") if "compressor" in self.model_name_prefix else f"{self.model_name_prefix}_sensor_motor"
        model_path = self.model_dir / f"{sensor_motor_prefix}_v{self.version}.pth"
    ...
    def train_all(self, num_samples=1000, epochs=20, batch_size=4):
        ...
        history_path = self.model_dir / f"training_history_{self.model_name_prefix}_v{self.version}.json"
...
def main():
    ...
    parser.add_argument('--model-name-prefix', type=str, default=None, help='Model name prefix (e.g., lydlr_compressor)')
    ...
    trainer = AdvancedCompressionTrainer(
