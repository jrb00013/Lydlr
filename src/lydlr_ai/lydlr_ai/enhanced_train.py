...
# Enhanced Training Script for Lydlr with all improvements
import os
import torch
...
class EnhancedTrainer:
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (image, lidar, imu, audio) in enumerate(progress_bar):
            ...
            # Ensure correct shapes for LiDAR - handle extra dimension
            if len(lidar.shape) == 5:  # [B, 1, T, N, 3] or [B, 1, T, N*3]
                # Remove extra dimension and flatten if needed
                lidar = lidar.squeeze(1)  # Remove extra dim
                if lidar.size(-1) == 3:  # If still 3D, flatten
                    lidar = lidar.view(lidar.size(0), lidar.size(1), -1)  # [B, T, N*3]
            elif len(lidar.shape) == 4:  # [B, T, N, 3]
                lidar = lidar.view(lidar.size(0), lidar.size(1), -1)  # [B, T, N*3]
            ...
            if len(image.shape) == 5:  # [B, 1, T, C, H, W]
                image = image.squeeze(1)  # [B, T, C, H, W]
            if len(lidar.shape) == 5:  # [B, 1, T, N]
                lidar = lidar.squeeze(1)  # [B, T, N]
            if len(imu.shape) == 5:  # [B, 1, T, 6]
                imu = imu.squeeze(1)  # [B, T, 6]
            if len(audio.shape) == 5:  # [B, 1, T, N]
                audio = audio.squeeze(1)  # [B, T, N]
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for image, lidar, imu, audio in self.data_loader:
                ...
                # Ensure correct shapes - handle extra dimension
                if len(lidar.shape) == 5:  # [B, 1, T, N, 3] or [B, 1, T, N*3]
                    lidar = lidar.squeeze(1)  # Remove extra dim
                    if lidar.size(-1) == 3:  # If still 3D, flatten
                        lidar = lidar.view(lidar.size(0), lidar.size(1), -1)  # [B, T, N*3]
                elif len(lidar.shape) == 4:
