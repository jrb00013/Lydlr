#!/usr/bin/env python3
"""Persistent opencode-lydlr polylogue bridge — listen + send via outbox file."""
...
PARTICIPANT = "opencode-lydlr"
URI = f"ws://127.0.0.1:7850/ws?room={ROOM}&id={PARTICIPANT}"
LOG = Path("/tmp/polylogue-opencode-bridge.log")
OUTBOX = Path("/tmp/polylogue-opencode-bridge-outbox.jsonl")
...
async def _outbox_reader(ws: websockets.WebSocketClientProtocol) -> None:
    ...
    while True:
        ...
        for line in raw[offset:].splitlines():
            ...
            if "to" not in payload or payload.get("to") is None:
                payload.setdefault("to", "cursor-lydlr")
